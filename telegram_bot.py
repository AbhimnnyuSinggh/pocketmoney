...
class TelegramBotHandler:
    ...
    def __init__(self, cfg: dict):
        ...
        # Signal history — {opp_type: deque of (Opportunity, formatted_msg)}
    ...
    def _select_signal(self, chat_id: str, sig_key: str):
        ...
        if past:
            ...
            for formatted_msg in past[-10:]:
                self._send(chat_id, formatted_msg)
                time.sleep(0.5)  # Respect rate limits

    def _get_history_for(self, sig_key: str) -> list[str]:
        """Get past formatted messages matching a signal type."""
        s = SIGNAL_TYPES[sig_key]
        if s["opp_types"] is None:
            # "all" — collect from every type
            all_msgs = []
            for msgs in self.history.values():
                all_msgs.extend(msgs)
            # Sort isn't possible (no timestamp in formatted), just return last N
            return list(all_msgs)[-10:]
        else:
            result = []
    ...
    def distribute_signals(self, opportunities: list[Opportunity], cfg: dict):
        ...
        for opp in filtered:
            ...
            with self._lock:
                self.history[opp.opp_type].append(msg)
