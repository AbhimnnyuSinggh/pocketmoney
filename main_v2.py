...
COOLDOWN = 600  # Don't re-alert same opportunity within 10 minutes


def _opp_key(opp: Opportunity) -> str:
    """Create a unique key for deduplication."""
    return f"{opp.opp_type}:{opp.title[:50]}:{opp.platforms}"


def filter_new_opportunities(opps: list[Opportunity]) -> list[Opportunity]:
    """Only return opportunities not seen recently."""
    now = time.time()
    new_opps = []
    for opp in opps:
        key = _opp_key(opp)
...
def run_cycle(cfg: dict, cycle: int) -> list[Opportunity]:
    ...
    new_opps = filter_new_opportunities(opportunities)

    if new_opps:
        logger.info(f"🚨 {len(new_opps)} NEW opportunities to alert!")
        send_opportunities_batch(new_opps, cfg)
    else:
        logger.info("No new opportunities this cycle")
